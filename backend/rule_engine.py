...
def _evaluate_condition(condition: dict, alert: RawAlert) -> bool:
    ...
    # Resolve field value from alert
    alert_data = alert.model_dump()
    # Also check raw_payload fields
...
async def evaluate(alert: RawAlert, db: AsyncSession) -> RuleEngineResult:
    ...
    stmt = (
        ...
        .where(
            (Rule.source_slug == None) | (Rule.source_slug == alert.source_slug)  # noqa: E711
        )
    )
    ...
    for rule in rules:
        try:
            ...
            logger.debug("Rule '%s' matched alert from %s", rule.name, alert.source_slug)
