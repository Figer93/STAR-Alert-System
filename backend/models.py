...
from sqlalchemy import Integer, String, Text, Boolean, DateTime, ForeignKey, JSON, Enum as SAEnum
...
class Source(Base):
    ...
    status: Mapped[str] = mapped_column(SAEnum("online", "offline", "unknown", name="source_status"), default="unknown")
...
class Alert(Base):
    ...
    severity: Mapped[str] = mapped_column(SAEnum("critical", "warning", "info", "ok", name="alert_severity"))
    title: Mapped[str] = mapped_column(String(255))
    message: Mapped[str] = mapped_column(Text)
    raw_payload: Mapped[dict] = mapped_column(JSON, default=dict)
    fingerprint: Mapped[str] = mapped_column(String(64), index=True)
    status: Mapped[str] = mapped_column(SAEnum("active", "acknowledged", "resolved", name="alert_status"), default="active")
    ...
    notification_logs: Mapped[list["NotificationLog"]] = relationship("NotificationLog", back_populates="alert")
...
class Rule(Base):
    ...
    severity_override: Mapped[str | None] = mapped_column(SAEnum("critical", "warning", "info", "ok", name="rule_severity"), nullable=True)
